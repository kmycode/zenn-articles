...
このケースは、継承を単に基底クラスの拡張であると見誤っている時によく発生します。プログラミング入門書ではよく「拡張」というフレーズが出てきます。文法の説明としては正しいですが、オブジェクト指向プログラミングの設計論を語る上では不十分で誤解を招く表現であると私は考えます。何度も繰り返しますが、確かに継承には拡張も含まれますが、継承のもう１つの機能であるオーバーライドとは実装の隠蔽であり、ブラックボックス化です。**継承、あるいは実装によって隠蔽された具象クラスのメソッドは、基底クラスのそれと差し替え可能でなければいけません**。これを遵守することは、また別のサブクラスとの置換が可能であることもまた意味します。なぜならサブクラスAも、サブクラスBも、同じ基底クラスと置換可能だからです。
