...
何度も繰り返しますが、継承や実装によってメソッドをオーバーライドまたは実装する行為とは具象の隠蔽であり、ブラックボックス化です。**継承、あるいは実装によって隠蔽された具象クラスのメソッドは、基底クラスのそれと差し替え可能でなければいけません**。これを遵守することは、抽象の信頼性を向上するだけでなく、別のサブクラスとの置換が可能であることもまた意味します。なぜならサブクラスAもBも同じ基底クラスと置換可能であり、三段論法が成立するからです。
