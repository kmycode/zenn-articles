...
以下のコードは常に`false`を返すべきです。なぜなら`yamata1`と`yamata2`はそれぞれ別のアドレス上に生成されたインスタンスだからです。
...
しかしメモリ上のサイズは、`string`は12バイト、`long`は8バイトになります。プログラムでの扱いやすさは`string`にも分があるかもしれませんが、メモリの関係で後から`long`に変更する可能性もあります。そうでなくても、例えば

```cs
  this.MyNumber = "なまえ";    // this.Nameと間違えた
```

というふうに、同じ`string`型である別の変数を誤って代入するリスクも考えられます。これはバグのもとになります。
したがって、マイナンバーそのものを型として切り出すことにします。
...
このように、値の中でも、`string`や`int`など既存の型で表現はできるもののフォーマットが決まっており他の変数と区別したい場合は、`struct`など新しい型を作って切り出せば、保守性に貢献するでしょう。これを値オブジェクトといいます。
...
値オブジェクトはあくまで値を示すべきであり、同一性はただそのデータ内容によってのみ決まります。したがって、`Yamata`クラスの`FirstName`プロパティを値オブジェクトにすることはできません。（同姓同名の人がいるため）
...
これらのメソッドは2つの型を縦断しますが、インスタンスメソッドとして作ろうとすると、2つの型のうちどちらのメソッドとして含めるべきか判断に迷います。そもそも別の型への変換がクラスの責務に含まれないこともしばしばです。どちらの型に含めるか非常に曖昧な時は、型の変換を専門とする`static class`なサービスクラスを作っておいたほうが楽なのです。
